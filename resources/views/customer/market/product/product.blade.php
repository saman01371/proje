                                ...
                                <section class="product-gallery">
                                    ...
                                    <section class="product-gallery-thumbs">
                                        @foreach ($images as $key => $image)
                                            ...
                                            <img class="product-gallery-thumb" src="{{ asset($image['indexArray']['medium']) }}"
                                        ...
                                        @if($colors->count() != 0)
                                            ...
                                            <p>
                                                @foreach ($colors as $key => $color)
                                                    ...
                                                    <input class="d-none" type="radio" name="color" id="{{ 'color_' . $color->id }}"
                                            ...
                                            <p><i class="fa fa-shield-alt cart-product-selected-warranty me-1"></i>
                                                ...
                                                <select name="guarantee" id="guarantee" class="p-1">
                                                    @foreach ($guarantees as $key => $guarantee)
                                                        <option value="{{ $guarantee->id }}" data-guarantee-price={{ $guarantee->
                                                price_increase }} @if($key == 0) selected @endif>{{ $guarantee->name }}
                                                        </option>
                                                    @endforeach
                            ...
                            <section class="content-wrapper bg-white p-3 rounded-2 cart-total-price">
                                <section class="d-flex justify-content-between align-items-center">
                                    ...
                                    <p class="text-muted"><span id="product_price" data-product-original-price={{
                                        $product->price }}>{{ priceFormat($product->price) }}</span> <span
                                ...
                                <table class="table table-bordered border-white">
                                    @foreach ($product->values()->get() as $value)
                                        <tr>
                                            ...
                                            <td>{{ json_decode($value->value)->value }} {{ $value->attribute()->first()->unit }}
                                            </td>
                                        </tr>
                                    @endforeach
                                ...
                                @foreach ($product->activeComments() as $activeComment)
                                    <section class="product-comment">
                                        ...
                                        @foreach ($activeComment->answers()->get() as $commentAnswer)
                                            <section class="product-comment">
                                                <section class="product-comment-header d-flex justify-content-start">
                                                    <section class="product-comment-date">{{ jalaliDate($commentAnswer->created_at)
                                            }}</section>
                                                    @php
...
{{--{{dd(auth()->user()->isUserPurchedProduct($product->id)->count())}}--}}
                            @auth
                                @if(auth()->user()->isUserPurchedProduct($product->id)->count() > 0)
                                    ...
                                    <section class="product-rating mb-4">
                                        ...
                                        <div class="container">
                                            ...
                                            <h6>
                                                امتیاز شما : {{ $product->ratingsCountUser() ?? 'شما اولین امتیاز را ثبت
                                    نمایید!!!' }}
                                            </h6>
                                            <h6>
                                    ...
                                    امتیاز را ثبت نمایید!!!' }}
                                            ...
                                            <h6>
                                                تعداد افراد شرکت کننده : {{ $product->ratingsCount() ?? 'شما اولین نفر برای ثبت امتیاز باشید
                                    !!!' }}
                                            </h6>
                                        </div>
